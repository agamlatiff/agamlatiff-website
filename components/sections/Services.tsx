    ...
    <section id="services" className="py-24 bg-slate-50 dark:bg-slate-950 transition-colors duration-300 relative overflow-hidden">
      ...
      <div className="max-w-[1200px] mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center max-w-3xl mx-auto mb-12">
          ...
          >
            Pilihan paket lengkap untuk operasional bisnis, branding, dan pemasaran online.
          </motion.p>
        </div>
        ...
        <div className="relative max-w-4xl mx-auto mb-12 group/slider px-8 md:px-12">
            ...
            <button 
                ...
                className={`absolute left-0 top-1/2 -translate-y-1/2 z-20 bg-white dark:bg-slate-800 p-2 rounded-full shadow-md border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 transition-all md:-translate-x-2 ${
                    !canScrollLeft 
            ...
            >
              <div className="flex gap-3 py-2 px-1">
                  ...
                  return (
                      ...
                      className={`flex-shrink-0 relative flex items-center gap-2 px-5 py-3 rounded-full text-sm font-bold transition-all duration-300 border whitespace-nowrap ${
                          isActive 
                      ...
                      <span className="relative z-10">{service.shortTitle || service.title}</span>
                      </button>
                  );
                  })}
              </div>
            ...
            <button 
                ...
                className={`absolute right-0 top-1/2 -translate-y-1/2 z-20 bg-white dark:bg-slate-800 p-2 rounded-full shadow-md border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 transition-all md:translate-x-2 ${
                    !canScrollRight 
                    ? 'opacity-50 cursor-not-allowed text-slate-300 dark:text-slate-600' 
                    : 'hover:bg-primary hover:text-white hover:border-primary'
                }`}
        ...
        <div>
            ...
            >
                ...
                <h3 className="text-xl md:text-2xl font-bold text-slate-900 dark:text-white mb-4">
                    Paket {activeServiceInfo?.shortTitle || activeServiceInfo?.title}
                </h3>
                    ...
                    <div className="p-6 sm:p-8 flex flex-col h-full relative overflow-hidden rounded-3xl">
                        ...
                        {plan.isPopular && (
                          <div className="absolute top-0 right-0">
                             <div className="bg-gradient-to-l from-primary to-indigo-600 text-white text-[10px] font-extrabold px-4 py-1.5 rounded-bl-2xl shadow-lg flex items-center gap-1.5 tracking-wide uppercase">
                                <Zap size={10} fill="currentColor" /> Best Choice
                             </div>
                          </div>
                        )}
                            ...
                            {plan.features.map((feat, idx) => (
                                <div key={idx} className="flex items-start gap-3 group/feat">
                                    ...
                                    <span className={`text-sm font-medium transition-colors ${
                                        plan.isPopular ? 'text-slate-700 dark:text-slate-200' : 'text-slate-600 dark:text-slate-400'
                                    }`}>
                                        {feat}
                                    </span>
                                </div>
                        ...
                        <div className="mt-auto space-y-3">
                            <a 
                                ...
                                className={`relative w-full flex items-center justify-center gap-2 py-4 rounded-xl font-bold text-sm transition-all duration-300 overflow-hidden group/btn ${
                                    plan.isPopular
            ...
            <div className="mt-16 text-center">
                <div className="inline-block p-6 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-100 dark:border-slate-800 max-w-2xl mx-auto hover:border-primary/20 transition-colors">
                    <p className="text-base text-slate-600 dark:text-slate-300 leading-relaxed">
                        Butuh <strong>Custom App</strong> atau <strong>Integrasi Payment Gateway</strong>? <br />
                        <a href={WA_LINKS.general} target="_blank" rel="noopener noreferrer" className="text-primary font-bold hover:underline inline-flex items-center gap-1 mt-2">
                           Hubungi saya untuk diskusi teknis & penawaran khusus &rarr;
                        </a>
                    </p>
                </div>
            </div>
