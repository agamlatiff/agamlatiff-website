...
const Contact: React.FC<ContactProps> = ({ onOpenChat }) => {
  ...
  return (
    <section id="contact" className="py-24 bg-white dark:bg-slate-950 transition-colors duration-300 border-t border-slate-200/60 dark:border-slate-800">
      <Toast 
        message={status === 'error' ? "Gagal Mengirim" : "Pesan Terkirim!"}
        subMessage={status === 'error' ? "Mohon cek koneksi atau hubungi via WhatsApp." : "Email Anda telah diterima. Saya akan segera membalasnya."}
        isVisible={showToast || status === 'error'}
             ...
             <div className="relative z-10">
               ...
               <div className="space-y-8">
                 <div className="flex items-center gap-4 group">
                   ...
                   <div>
                     <div className="text-xs text-indigo-200 dark:text-slate-500 uppercase tracking-wider font-semibold mb-1">Email</div>
                 ...
                 <div className="flex items-center gap-4 group">
                   ...
                   <div>
                     <div className="text-xs text-indigo-200 dark:text-slate-500 uppercase tracking-wider font-semibold mb-1">Status</div>
                 ...
                 <button onClick={onOpenChat} className="flex items-center gap-4 group w-full text-left">
                   ...
                   <div>
                     <div className="text-xs text-indigo-200 dark:text-slate-500 uppercase tracking-wider font-semibold mb-1">AI Assistant</div>
          ...
          <div className="p-8 lg:p-12 lg:w-3/5 bg-slate-50/50 dark:bg-slate-900">
             <form onSubmit={handleSubmit} className="space-y-6" noValidate>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                  <div>
                    <label htmlFor="name" className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2 transition-colors">Nama Lengkap</label>
                    <input 
                        type="text" 
                        id="name" 
                        value={formData.name}
                  ...
                  <div>
                    <label htmlFor="phone" className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2 transition-colors">No. WhatsApp</label>
                ...
                {/* Email (Full width) */}
                <div>
                    <label htmlFor="email" className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2 transition-colors">Alamat Email</label>
                    <input 
                        type="email" 
                        id="email" 
                ...
                <div>
                  <label htmlFor="type" className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2 transition-colors">Jenis Layanan</label>
                  <div className="relative">
                      <select 
                        id="type" 
                        value={formData.type}
                      ...
                      >
                        <option className="bg-white dark:bg-slate-900 text-slate-900 dark:text-white">Sistem POS & Inventory (Kasir)</option>
                        <option className="bg-white dark:bg-slate-900 text-slate-900 dark:text-white">Sistem Booking / Reservasi Online</option>
                        <option className="bg-white dark:bg-slate-900 text-slate-900 dark:text-white">Website Company Profile (CMS)</option>
                        <option className="bg-white dark:bg-slate-900 text-slate-900 dark:text-white">Landing Page Promosi</option>
                        <option className="bg-white dark:bg-slate-900 text-slate-900 dark:text-white">Toko Online / E-Commerce</option>
                        <option className="bg-white dark:bg-slate-900 text-slate-900 dark:text-white">Dashboard Admin Custom</option>
                        <option className="bg-white dark:bg-slate-900 text-slate-900 dark:text-white">Lainnya / Custom Request</option>
                      </select>
                ...
                <div>
                  <label htmlFor="message" className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2 transition-colors">Pesan Anda</label>
                  <textarea 
                    id="message" 
                    rows={4} 
                    ...
                    placeholder="Halo Mas Agam, saya butuh sistem POS untuk [Jenis Bisnis]..."
                  ></textarea>
