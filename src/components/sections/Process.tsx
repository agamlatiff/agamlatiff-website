...
const Process: React.FC = () => {
  ...
  return (
    <section id="services" className="py-24 bg-slate-950 transition-colors duration-300 border-t border-slate-800 overflow-hidden">
      <div className="max-w-[1200px] mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <h2 className="text-3xl md:text-4xl font-bold text-white mb-6 transition-colors">
            {t('process.title')}
          </h2>
          <p className="text-slate-400 text-lg leading-relaxed transition-colors">
        ...
        <div className="relative">
          ...
          <div className="hidden md:block absolute left-1/2 top-0 bottom-0 w-0.5 bg-gradient-to-b from-slate-800 via-primary/30 to-slate-800 -translate-x-1/2 rounded-full z-0"></div>

          {/* Left Line (Mobile) */}
          <div className="md:hidden absolute left-8 top-0 bottom-0 w-0.5 bg-gradient-to-b from-slate-800 via-primary/30 to-slate-800 rounded-full z-0"></div>
                ...
                >
                  ...
                  <div className="flex-1 w-full pl-20 md:pl-0">
                    ...
                    <div className={`w-full ${isLeft ? 'md:pr-12 md:text-right' : 'md:pl-12 md:text-left'}`}>
                      ...
                      <div className="relative p-6 md:p-8 bg-slate-900 rounded-2xl border border-slate-800 shadow-sm hover:shadow-xl hover:border-primary/30 transition-all duration-300 group z-10">

                        {/* Connector Arrow (Desktop Only) */}
                        <div className={`hidden md:block absolute top-8 w-4 h-4 bg-slate-900 border-t border-r border-slate-800 transform rotate-45 ${isLeft ? '-right-2.5' : '-left-2.5 border-r-0 border-t-0 border-b border-l'}`}></div>
                        ...
                        <div className={`bg-slate-950/50 rounded-xl p-4 border border-slate-800/50`}>
                          <h4 className={`text-xs font-bold text-slate-400 uppercase tracking-wide mb-3 ${isLeft ? 'md:text-right' : 'md:text-left'}`}>Output / Deliverables:</h4>
                          <div className={`flex flex-col gap-2.5 ${isLeft ? 'md:items-end' : 'md:items-start'}`}>
                            {(tStep?.deliverables || step.deliverables).map((item, idx) => (
                              <div key={idx} className={`flex items-center gap-2 text-sm text-slate-300 ${isLeft ? 'md:flex-row-reverse' : ''}`}>
                  ...
                  <div className="absolute left-8 md:left-1/2 top-0 md:top-8 -translate-x-1/2 flex items-center justify-center z-20">
                    <div className="relative w-12 h-12 md:w-16 md:h-16 bg-slate-900 rounded-full border-4 border-slate-800 flex items-center justify-center shadow-lg group-hover:scale-110 transition-transform duration-300">
                    ...
                    <div className="md:hidden absolute top-1/2 left-10 w-6 h-0.5 bg-slate-800"></div>
                  </div>
        ...
        >
          <div className="inline-block p-1 bg-slate-800 rounded-full mb-6">
            <div className="bg-slate-900 rounded-full px-6 py-2 text-sm text-slate-400 border border-slate-700">
